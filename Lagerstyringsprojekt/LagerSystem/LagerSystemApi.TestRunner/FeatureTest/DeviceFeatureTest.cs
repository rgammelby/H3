...
namespace LagerSystemApi.TestRunner.FeatureTest
{
    public class DeviceFeatureTest
    {
        ...
        [Fact]
        public async void T_GetSingleDevicesByModel()
        {
            var repository = new FakeDeviceRepository();
            string searchModel = "iPhone";

            var result = await repository.GetSingleDevicesByModel(searchModel);

            Assert.NotNull(result);
            Assert.NotEmpty(result);
            Assert.All(result, d =>
                Assert.Contains("iPhone", repository
                    .DeviceOverviews
                    .FirstOrDefault(o => o.id == d.device_overview_id)
                    ?.model
                )
            );
        ...
        [Fact]
        public async void T_GetSingleDevicesByType()
        {
            var repository = new FakeDeviceRepository();
            string searchType = "Laptop";

            var result = await repository.GetSingleDevicesByType(searchType);

            Assert.NotNull(result);
            Assert.NotEmpty(result);
            Assert.All(result, d =>
                Assert.Contains("Laptop", repository
                    .DeviceTypes
                    .FirstOrDefault(t => t.device_id == repository
                        .DeviceOverviews
                        .FirstOrDefault(o => o.id == d.device_overview_id)?.device_type)
                    ?.device_type
                )
            );
        }
        ...
        //   {
        //       id = 1,
        //       device_overview_id = "De, description = "This is a Monitor", location_id = 1, is_archived = 1, qr = "Device - id = 1", status = 0 };
